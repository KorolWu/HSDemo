...
//    QAction *defaultLanguageAction = languagesMenu->addAction(tr("English - US"));
...
//    QAction *chinaLanguageAction = languagesMenu->addAction("China");
//    chinaLanguageAction->setProperty(FILE_ON_DISK_DYNAMIC_PROPERTY, qApp->applicationDirPath() + LANGUAGES_SUBFOLDER + "tr_zh.ts");
//    connect(chinaLanguageAction, SIGNAL(triggered(bool)), this, SLOT(onLanguageActionTriggered(bool)));

    // Load all languages and populate the menus
    QDir languagesDir(qApp->applicationDirPath() + LANGUAGES_SUBFOLDER);
    QFileInfoList languageFiles = languagesDir.entryInfoList(QStringList() << "*.ts", QDir::NoDotAndDotDot | QDir::Files, QDir::Name);
...
void MainWindow::populateThemesMenu()
{
    ...
    QAction *defaultThemeAction = themesMenu->addAction(tr("默认"));
    ...
    QAction *drayThemeAction = themesMenu->addAction((tr("暗黑")));
    drayThemeAction->setProperty(FILE_ON_DISK_DYNAMIC_PROPERTY,qApp->applicationDirPath() + THEMES_SUBFOLDER + "Dark.thm");
    connect(drayThemeAction, SIGNAL(triggered(bool)), this, SLOT(onThemeActionTriggered(bool)));
...
{
    processedPixmap.setPixmap(QPixmap(""));
    m_lastProcessResultMat.release();
    m_isNewPlugin = false;
    ...
    if(!originalMat.empty())
    {
        if(!currentPlugin.isNull())
        {
            ...
            if(currentPluginInstance)
            {
                cv::TickMeter meter;
                meter.start();
                if(!m_lastProcessResultMat.empty() && m_isNewPlugin == true)
                {
                    m_isNewPlugin = false;
                    currentPluginInstance->processImage(m_lastProcessResultMat, processedMat);
                    qDebug()<<"change plugin srcImage = lastProcessImage";
                      imshow("changePlugin",m_lastProcessResultMat);
                }
                else
                {
                    currentPluginInstance->processImage(originalMat, processedMat);
                    imshow("org",originalMat);
                    qDebug()<<"srcImage = srcImage";
                }
                //add from korol
                m_lastProcessResultMat  = processedMat.clone();
                meter.stop();
